...
нужна, поэтому старые файлы остаются и читаются маской `<имя>_checks_*.parquet`.
"""
...
def run(name: str, title: str, preamble: str, body: str, paths, error_text: str) -> list:
    ...
    `name` — префикс файлов журнала (`<name>_checks_<время>.parquet`),
    ...
    journal_file = paths.checks / f'{name}_checks_{stamp}.parquet'
